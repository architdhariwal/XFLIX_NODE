...
const generateAuthTokens = async (user) => {
  const accessTokenExpiration = Math.floor(Date.now()/1000)+config.jwt.accessExpirationMinutes;
  const accessToken = generateToken(user._id, accessTokenExpiration, tokenTypes.ACCESS)
  return {
    access: {
      token: accessToken,
